...
//   export default function NewArrivals() {
//   const { cart, addToCart } = useCart();   // ⚠️ idha add pannunga
//   const [picked, setPicked] = useState({});
//   // ⚠️ munnadi irundha "const [cart, setCart] = useState([]);" line-a DELETE pannunga
  ...
  return (
    <section className="na-section">
      ...
      <div className="na-grid">
        {newProducts.map((p) => {
          const off = Math.round(((p.mrp - p.price) / p.mrp) * 100);
          const idx = picked[p.id] ?? 0;
          const current = p.colors[idx];
          ...
          return (
            <div className="na-card" key={p.id}>
              ...
              <div className="na-info">
                ...
                <button
                  className={inCart ? "na-btn added" : "na-btn"}
                  onClick={() => addToCart({ id: p.id, name: p.name, price: p.price, img: current.img })}
                >
